...
sys.path.insert(0,'..')
# sys.path.insert(0,'../..')
# sys.path.insert(0,'../../pytorch/micro_batch_train_prune/')
...
from block_dataloader_prune import generate_dataloader_block
from graphsage_prune import GraphSAGE
from gcn_prune import GCN
from gat_prune import GAT
...
def run(args, device, data):
	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			time_ex=0
			tts=time.time()
			reuse_embedding = torch.tensor([],dtype=torch.float32) #!
			for step, (input_nodes, seeds, blocks, prev_layer_repeat) in enumerate(block_dataloader):
				...
				tt3=time.time()
				#! temp tensor for concat out and reuse
				#temp_emb=torch.full((50000, 256), 0, dtype=torch.float32, device=device)
				blocks_and_repeat=[blocks, prev_layer_repeat, step] #!
				input_reuse=[batch_inputs,reuse_embedding.to(device)]
				batch_pred,cache_embedding= model(blocks_and_repeat, input_reuse) #!
				reuse_embedding = cache_embedding #!
			...
			if args.eval:
				...
				print("Run {:02d} | Epoch {:05d} | Loss {:.4f} | Train {:.4f} | Val {:.4f} | Test {:.4f}".format(run, epoch, loss_sum, train_acc, val_acc, test_acc))
			else:
...
#--------------------------------------------------------------------------------------
	...
	argparser.add_argument('--topx', type=bool, default=True)
