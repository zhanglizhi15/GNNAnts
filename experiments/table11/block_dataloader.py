...
from graph_partitioner_topx import Graph_Partitioner #! for betty+topx
...
def generate_one_block(raw_graph, global_srcnid, global_dstnid, global_eids):
	...
	print("new_block.srcdata:", len(new_block.srcdata[dgl.NID]), len(global_srcnid))
...
def check_connections_block(batched_nodes_list, current_layer_block):
	...
	print('src_nid_list ', len(src_nid_list))
	...
	for step, output_nid in enumerate(batched_nodes_list):
		...
		tt1=time.time()
		#in_degrees=current_layer_block.in_degrees()
		#no_zerodegree=torch.count_nonzero(in_degrees).item()
		#print("degrees:", no_zerodegree,in_degrees.size())
		#current_layer_block=dgl.add_self_loop(current_layer_block)
		local_in_edges_tensor = current_layer_block.in_edges(local_output_nid, form='all')
		#! exist 0 degree nodes!!!
		#print('local_in_edges_tensor', len(local_in_edges_tensor))
		...
		ttpp=time.time()
		#print('mini_batch_src_local', mini_batch_src_local, len(mini_batch_src_local))
		mini_batch_src_local = list(OrderedDict.fromkeys(mini_batch_src_local.tolist()))
		print('mini_batch_src_local', len(mini_batch_src_local))
		...
		if set(mini_batch_dst_local.tolist()) != set(local_output_nid):
			print('local dst not match', len(set(mini_batch_dst_local.tolist())), len(set(local_output_nid)))
		eid_local_list = list(local_in_edges_tensor)[2] # local (𝑈,𝑉,𝐸𝐼𝐷); 
		global_eid_tensor = eids_global[eid_local_list] # map local eid to global.
	# 	# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  bottleneck  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
		...
		#! check topk
		#print("src_nid", src_nid, len(src_nid))
...
def gen_grouped_dst_list(prev_layer_blocks):
	post_dst=[]
	print("prev_layer_blocks",prev_layer_blocks)
...
def generate_dataloader_gp_block(raw_graph, full_block_dataloader, args):
	...
	for batch_id in range(num_batch):
		...
		nsrc = nsrc + len(cur_blocks[0].srcdata[dgl.NID])
