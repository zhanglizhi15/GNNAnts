...
from graph_partitioner import Graph_Partitioner #! for betty+reuse
#from graph_partitioner_topx import Graph_Partitioner
...
def check_connections_block(batched_nodes_list, current_layer_block):
	...
	for step, output_nid in enumerate(batched_nodes_list):
		...
		tt1=time.time()
		#? the node id in layer_block is local
		local_in_edges_tensor = current_layer_block.in_edges(local_output_nid, form='all')  # three tensor: src, dst, and edges id
		...
		#print('local_global', len(mini_batch_src_local),len(induced_src))
		mini_batch_src_global= induced_src[mini_batch_src_local].tolist() # map local src nid to global.
...
# remove the repeated nodes from src 
def check_connections_pruned_src_block(batched_nodes_list, current_layer_block, args):
	str_=''
	res=[]
	# print('check_connections_block*********************************')
	...
	#graph_table = torch.zeros((169343), dtype=torch.int)  #! all the graph nodes

	#max_repeat = 0
	#ord_seed = numpy.arange(args.num_batch)
	#for i in range(30):
	all_repeat=0
	#if i!=0:
		#random.shuffle(ord_seed)   #! random find optimal solution
	#print("ord_seed:", ord_seed)
	src_nid_prev_batch = [] #keep the src of the prev batch
	repeat_in_subbatch = []
	#print("batched_nodes_list", len(batched_nodes_list))
	for step, output_nid in enumerate(batched_nodes_list):
	#for step in range(args.num_batch):
		#seed = ord_seed[step]
		#output_nid = batched_nodes_list[seed]
		# in current layer subgraph, only has src and dst nodes,
		# and src nodes includes dst nodes, src nodes equals dst nodes.
		tt=time.time()
		local_output_nid = list(map(dict_nid_2_local.get, output_nid))
	...
	# 	# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  bottleneck  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
		...
		# print('before src:', len(r_))
		# repeated_nodes, indice1, indice2 = numpy.intersect1d(r_, src_nid_prev_batch,assume_unique=True,return_indices=True) 
		# repeat_in_subbatch.append(repeated_nodes)
		# src_nid_prev_batch = r_ 
		# print("repeated_nodes", len(repeated_nodes),len(r_))
		# print("output_nid", type(output_nid), type(r_))

		src_nid = torch.tensor(output_nid + r_, dtype=torch.long)

		#! record the repeated nodes from src 
		print('before src:', src_nid.size())
		repeated_nodes, indice_cache, indice_reuse = numpy.intersect1d(src_nid_prev_batch, src_nid.numpy(), assume_unique=True,return_indices=True)
		#sec_nid = numpy.setdiff1d(src_nid, repeated_nodes, assume_unique=True)
		src_nid_prev_batch = src_nid 
		all_repeat+=len(repeated_nodes)
		print("repeated_nodes", len(repeated_nodes))
		
		repeat_indice=[indice_cache, indice_reuse]
		output_nid = torch.tensor(output_nid, dtype=torch.long)

		res.append((src_nid, output_nid, global_eid_tensor, repeat_indice))

		#if all_repeat>max_repeat:
			#max_repeat=all_repeat
	#print(f"all_repeat: {all_repeat}, cur_max_repeat: {max_repeat}")
...
# prune the repeated nodes from src
def generate_src_pruned_layer_block(raw_graph, layer_block, batches_nid_list, args):
	...
	t1= time.time()
	batches_temp_res_list = check_connections_pruned_src_block(batches_nid_list, layer_block, args)
	t2 = time.time()
	check_connection_time.append(t2-t1)
	...
	for step, (srcnid, dstnid, current_block_global_eid, repeat_indice) in enumerate(batches_temp_res_list):
		t_ = time.time()
		cur_block = generate_one_block(raw_graph, srcnid, dstnid, current_block_global_eid) # block -------
		t__=time.time()
		block_generation_time.append(t__-t_) 
		...
		blocks.append(cur_block)
		#print("srcnid", type(srcnid))
		#srcnid = numpy.setdiff1d(srcnid, repeated_nodes, assume_unique=True).tolist()   #! pruned 
		src_list.append(srcnid)
		dst_list.append(dstnid)
		repeat_list.append(repeat_indice)

	connection_time = sum(check_connection_time)
	block_gen_time = sum(block_generation_time)

	return blocks, src_list, dst_list, repeat_list, (connection_time, block_gen_time)
...
def gen_grouped_dst_list(prev_layer_blocks,prev_layer_repeat):
	post_dst=[]
	for block in prev_layer_blocks:
		src_nids = block.srcdata['_ID'].tolist() #? tolist()
		post_dst.append(src_nids)
	return post_dst # return next layer's dst nids(equals prev layer src nids)

def gen_grouped_prune_dst_list(prev_layer_blocks,prev_layer_repeat):
	post_dst=[]

	for batch, block in enumerate(prev_layer_blocks):
		#print("btbk", batch)
		src_nids = block.srcdata['_ID'] #.tolist() #? tolist()
		#src_nid2 = block.srcdata[dgl.NID].tolist()
		#dst_nid = block.dstdata[dgl.NID].tolist()
		#print("some_len", len(src_nids),len(src_nid2),len(dst_nid))

		indices = prev_layer_repeat[batch][1].tolist()
		#print("repeat_nid", len(indices),len(src_nids))
		#print("prev_repeat_indice", indices,type(indices),type(src_nids))
		#print("repeat_nodes",batch-1,batch,src_nids[indices])
		src_nids = numpy.delete(src_nids, indices).tolist()     #! pruned the repeated nodes from src
		#if len(prev_layer_repeat)>0:
			#src_nids = numpy.setdiff1d(src_nids, prev_layer_repeat[batch], assume_unique=True).tolist()   #! pruned 
	...
	for _,(src_full, dst_full, full_blocks) in enumerate(full_block_dataloader): # only one full batch blocks
		...
		for layer_id, layer_block in enumerate(reversed(full_blocks)):
			...
			if layer_id ==0:
				...
				tmm=time.time()
				grouped_output_nid_list=gen_grouped_dst_list(prev_layer_blocks)  #many micro-batch
...
def generate_dataloader_gp_block(raw_graph, full_block_dataloader, args):
	...
	#b it means the graph partition starts 
	# from the output layer to the first layer input block graphs.
	for _,(src_full, dst_full, full_blocks) in enumerate(full_block_dataloader): #? only one full batch blocks; get the output mini-batch of the original dataloader
		
		block_gen_time_total=0
		l=len(full_blocks)

		#for i, blks in enumerate(full_blocks):
			#print('blocks', blks)

		for layer_id, layer_block in enumerate(reversed(full_blocks)):  #? largest block to the first order
			#print('rblocks', layer_block)
			# print('layer id ', layer_id)
			...
			if layer_id ==0:
				...
				# block 0 : (src_0, dst_0); block 1 : (src_1, dst_1);.......
				#blocks, src_list, dst_list, time_1 = generate_blocks_for_one_layer_block(raw_graph, layer_block,  batched_output_nid_list)
				blocks, src_list, dst_list, repeat_list, time_1 = generate_src_pruned_layer_block(raw_graph, layer_block,  batched_output_nid_list, args)
				
				prev_layer_blocks=blocks
				prev_layer_repeat=repeat_list
				blocks_list.append(blocks)
				final_dst_list=dst_list
				if layer_id==args.num_layers-1:
					final_src_list=src_list
				...
				tmm=time.time()
				if layer_id == 1:
					grouped_output_nid_list=gen_grouped_prune_dst_list(prev_layer_blocks,prev_layer_repeat)
				else:
					grouped_output_nid_list=gen_grouped_dst_list(prev_layer_blocks,prev_layer_repeat)
				# print('gen group dst list time: ', time.time()-tmm)
				num_batch=len(grouped_output_nid_list)
				# print('num of batch ',num_batch )
				blocks, src_list, dst_list, time_1 = generate_blocks_for_one_layer_block(raw_graph, layer_block, grouped_output_nid_list)
	...
	for batch_id in range(num_batch):
		...
		dst = final_dst_list[batch_id]
		src = final_src_list[batch_id]
		data_loader.append((src, dst, cur_blocks, prev_layer_repeat))
